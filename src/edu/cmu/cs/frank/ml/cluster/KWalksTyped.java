 ...
 * This one makes sure that probability is shared among nodes of the same types.
 * 
 * @author Frank Lin
 */

public class KWalksTyped implements Clusterer{

	protected static Logger log=Logger.getLogger(KWalksTyped.class);
	...
	private static Map<String,DoubleMatrix1D> getTypeMap(List<? extends Instance> instances){
		Map<String,DoubleMatrix1D> typeMap=new HashMap<String,DoubleMatrix1D>();
		for(int i=0;i<instances.size();i++){
			String id=instances.get(i).getId();
			int delim=id.indexOf(':');
			if(delim<1){
				log.error("Cannot find find type for instance "+id);
			}
			else{
				String type=instances.get(i).getId().substring(0,delim);
				DoubleMatrix1D mask=typeMap.get(type);
				if(mask==null){
					mask=new SparseDoubleMatrix1D(instances.size());
					typeMap.put(type,mask);
				}
				mask.set(i,1.0);
			}
		}
	...
	public List<Label> cluster(Dataset dataset){
		...
		log.info("Finding types...");
		
		Map<String,DoubleMatrix1D> typeMap=getTypeMap(dataset.getInstances());
		
		log.info("Types found: "+typeMap.keySet());
		...
		log.info("Matrix: "+w.rows()+" x "+w.columns());
		...
		for(int i=0;i<p.maxRounds&&!converged(centers,prevCenters);i++){
			...
			log.info("Most central nodes:");
			for(int j=0;j<centers.length;j++){
				int index=MatrixUtil.getMaxIndex(centers[j]);
				log.info(j+"th Centeral node: Index="+index+" ID="+dataset.getInstance(index).getId());
			}
			...
			for(int j=0;j<p.k;j++){
				...
				walks[j]=MatrixUtil.getPageRankVector(w,start,p.restartProb,p.randomWalkT,p.convergence,typeMap.values());
			}
			...
			for(int j=0;j<p.k;j++){
				...
				DoubleMatrix1D rank=MatrixUtil.getPageRankVector(masked,u,p.teleProb,p.pageRankT,p.convergence,typeMap.values());
				centers[j]=rank;
			}
