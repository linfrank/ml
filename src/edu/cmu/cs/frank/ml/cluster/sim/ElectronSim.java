...
public class ElectronSim implements Similarity{
	
	static Logger log=Logger.getLogger(ElectronSim.class);
	...
	public double measure(Instance a,Instance b){
		if(a.getFeatureSet()!=b.getFeatureSet()){
			...
			return Math.exp(-sum/(a.getFeatureSet().size()*p.s1*p.s1))-p.s2;
		}
	}
