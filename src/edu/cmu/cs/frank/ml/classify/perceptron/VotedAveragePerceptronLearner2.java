...
public class VotedAveragePerceptronLearner2 extends PerceptronLearner{
	
	static final long serialVersionUID=20071115L;
	
	protected static Logger logger=Logger.getLogger(VotedAveragePerceptronLearner2.class);
	...
	public VotedAveragePerceptronLearner2(int numEpochs,int maxPerceptrons){
		super(numEpochs);
		...
		for(Instance instance:instances){
			...
			if(prediction*truth>0){
				...
				if(perceptrons.size()>maxPerceptrons){
					...
					// average the perceptrons and give it the smallest weight
