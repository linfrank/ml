...
public class VotedAveragePerceptronLearner3 extends PerceptronLearner{
	
	static final long serialVersionUID=20071115L;
	
	protected static Logger logger=Logger.getLogger(VotedAveragePerceptronLearner3.class);
	...
	public VotedAveragePerceptronLearner3(int numEpochs,int maxPerceptrons){
		super(numEpochs);
		...
		for(Instance instance:instances){
			...
			if(prediction*truth>0){
				...
				if(perceptrons.size()>maxPerceptrons){
					...
					// average the perceptrons and give it the combined weight
					Perceptron averaged=Perceptron.average(current.vector.length,averaging,smallestWeight*averaging.size());
					perceptrons.add(averaged);
				}
